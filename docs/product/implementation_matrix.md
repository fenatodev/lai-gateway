...
| model local | experimental | endpoint local/private OpenAI-compatible | CLI/API/UI restritos | [módulo](../../lai_gateway/model.py), [testes](../../tests/test_model.py) | `model-chat` não cria Harness run, não usa nuvem, não executa tools, não instala runtime e não baixa modelo | PR97 memory_context |
