...
| adapter dispatcher | experimental | local restrito | CLI/API/UI restritos | [dispatcher](../../lai_gateway/adapter_dispatcher.py), [testes](../../tests/test_adapter_dispatcher.py) | Dispatch local_status.status exige local-status-read efetivo; sem shell, rede, credenciais ou filesystem write | PR95 recovery/replay |
...
Em `effective_authorization.py`, os escopos efetivos continuam estreitos: `adapter-dry-run` para simulação e `local-status-read` somente para `local_status.status`. Em `adapter_dispatcher.py`, dispatch real de `local_status.status` exige decisão allow, capability allowlisted, identidade verificada, escopo `local-status-read` e dispatch explícito. A confirmação da UI não comprova aprovação durável no backend. Não promover esse caminho a prova de autorização real ampla.
