...
- **implementação read-only** — [PR135](pr_135_governed_conversational_router.md): `governed-conversational-routing/v1` classifica linguagem natural em seis rotas advisory sem executar, autorizar ou iniciar Harness.
- **agente local de desenvolvimento read-only** — [PR136](pr_136_local_dev_agent.md): `local-dev-agent/v1` conecta o modelo local a read/search/status/diff governados, com tool calls estruturados, contexto limitado e sem Harness ou escrita.
