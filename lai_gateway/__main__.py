...
def main(argv: list[str] | None = None) -> int:
    ...
    public_browser_parser = sub.add_parser("public-browser", help="plan, fetch, or inspect one bounded public read-only URL")
    public_browser_parser.add_argument("--url", required=True, help="public http/https URL without credentials")
    public_browser_parser.add_argument("--browser-action", choices=["plan", "fetch", "extract", "inspect"], default="plan", help="plan without network, perform one public GET, or inspect the fetched source")
    ...
    persisted_audit_parser.add_argument("--actor", default=None, help="audit actor label; defaults to user")
    persisted_audit_parser.add_argument("--channel", default=None, help="audit channel label; defaults to gateway")
    persisted_audit_parser.add_argument("--domain", default=None, help="audit domain label; defaults to unknown")
    persisted_audit_parser.add_argument("--action", default=None, help="human-readable action label; stored only as a digest")
    persisted_audit_parser.add_argument("--param", action="append", default=None, help="bounded public parameter as key=value; repeatable")
    persisted_audit_parser.add_argument("--approve", action="store_true", help="mark approval intent before building the record")
    persisted_audit_parser.add_argument("--approved-by", default=None, help="bounded approver label")
    persisted_audit_parser.add_argument("--operation-scope", default=None, help="effective authorization scope; defaults to adapter-dry-run")
    ...
    local_task_dry_run_parser = sub.add_parser("local-task-dry-run", help="render a local-task/v1 dry-run proposal without executing it")
    local_task_dry_run_parser.add_argument("--task-id", default=None, help="local task id label")
    local_task_dry_run_parser.add_argument("--domain", default=None, help="task domain label")
    local_task_dry_run_parser.add_argument("--channel", default=None, help="task channel label")
    local_task_dry_run_parser.add_argument("--autonomy-zone", choices=["green", "yellow", "red"], default="green", help="task autonomy zone")
    local_task_dry_run_parser.add_argument("--capability", default=None, help="requested capability label")
    local_task_dry_run_parser.add_argument("--intent", default=None, help="short task intent")
    local_task_dry_run_parser.add_argument("--allowed-path", action="append", default=None, help="allowed path pattern; repeatable")
    local_task_dry_run_parser.add_argument("--denied-path", action="append", default=None, help="denied path pattern; repeatable")
    ...
    try:
        ...
        if args.command == "persisted-audit-log":
            ...
            if not args.json:
                print(render_persisted_audit_log(payload))
                return 0
            print(json.dumps(payload, indent=2, sort_keys=True))
            return 0
        ...
        if args.command == "config":
            ...
            if not args.json:
                ...
                return 0 if payload["overall"] == "ready" else 1
        elif args.command == "local-task-dry-run":
