...
- a local CLI for `config`, `contract`, `status`, `readiness`, `doctor`, `identity-binding`, `permission-ux`, `authorization-recovery`, `model-chat`, `health-report`, `open-ui`, `stack-start`, `mcp`, `sessions`, and `runs`, including run-event timeline reads;
