...
class AdapterRegistryTest(unittest.TestCase):
    ...
    def test_n8n_adapter_local_plan_is_registered_without_workflow_execution(self) -> None:
        ...
        self.assertEqual(adapter["autonomy"], "governed_local_plan")
        self.assertEqual(adapter["entrypoints"], ["/v1/gateway/n8n-local-plan"])
        self.assertEqual(adapter["granted_capabilities"], ["n8n.local_plan_digest"])
        self.assertTrue(adapter["local_handler_registered"])
        self.assertEqual(adapter["operation_scope"], "n8n-local-plan")
        self.assertTrue(adapter["requires_policy_check"])
        self.assertFalse(adapter["executes_tools"])
        self.assertFalse(adapter["workflow_execution_enabled"])
        self.assertFalse(adapter["workflow_activation_enabled"])
        self.assertFalse(adapter["webhook_execution_enabled"])
        self.assertFalse(adapter["calls_n8n_instance"])
        self.assertFalse(adapter["network_access_enabled"])
        self.assertFalse(adapter["credentialed_access_enabled"])
        self.assertFalse(adapter["shell_execution_enabled"])
        self.assertFalse(adapter["external_side_effects_enabled"])
