...
class GatewayUITest(unittest.TestCase):
    def test_gateway_serves_local_ui_with_security_headers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp, fake_harness() as harness:
            ...
            with RunningGateway(config) as gateway:
                ...
                self.assertIn('Health not loaded yet.', html)
                ...
                self.assertIn('Local chat & work runs', html)
                ...
                self.assertIn('Three local workbench operating modes', html)
                self.assertIn('Use Observe preset', html)
                self.assertIn('Use Work preset', html)
                self.assertIn('Use Promote preset', html)
    ...
    def test_gateway_serves_assets_without_external_dependencies_or_token_storage(self) -> None:
        ...
        self.assertIn("All systems ready", js)
